...
class _ReadAndWriteDemoState extends State<ReadAndWriteDemo> {
  ...
  Future<int> _readCounter() async {
    try {
      ...
      // 返回文件中的点击数 file.readAsString int.parse(content) 进行类型转换
  ...
  Future<File> _getLocalFile() async {
    // 获取本地文档目录 getApplicationDocumentsDirectory()
    String dir = (await getApplicationDocumentsDirectory()).path;
    // 返回本地文件目录 '$dir/counter.txt' 根目录下counter.txt文件 读取出来之后再进行写入
  ...
  Future<Null> _incrementCounter() async {
    ...
    // 将存储点击数的变量作为字符串写入文件
    await (await _getLocalFile()).writeAsString('$_counter');//file.readAsString  file.writeAsString 
  }
  @override
  Widget build(BuildContext context) {
    return new Scaffold(
      ...
      floatingActionButton: new FloatingActionButton(//右下角对应的按钮 floatingActionButton:new FloatingAction
